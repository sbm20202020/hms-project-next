...
import { useCreateExamenLabo } from '@/hooks/use-labo'
import { usePatients } from '@/hooks/use-patients'
import { useEmployes } from '@/hooks/use-employes'
import { toast } from 'sonner'
import { Loader2 } from 'lucide-react'

const TYPES_EXAMEN = ['Hémogramme', 'Glycémie', 'Bilan lipidique', 'Bilan hépatique', 'Bilan rénal', 'Ionogramme', 'CRP', 'Hémoculture', 'ECBU', 'Sérologie', 'Autre']

export function ExamenLaboForm({ onSuccess }: { onSuccess?: () => void }) {
  const create = useCreateExamenLabo()
  const { data: patients = [] } = usePatients()
  const { data: employes = [] } = useEmployes()
  const [form, setForm] = useState({ patientId: '', typeExamen: '', notes: '', demandeParId: '' })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.patientId || !form.typeExamen) { toast.error('Champs requis manquants'); return }
    create.mutate({
      patientId: Number(form.patientId), typeExamen: form.typeExamen,
      notes: form.notes || undefined, demandeParId: form.demandeParId ? Number(form.demandeParId) : undefined,
    } as any, {
      onSuccess: () => { toast.success('Examen labo créé'); onSuccess?.() },
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-foreground mb-1">Type d&apos;examen *</label>
        <select value={form.typeExamen} onChange={e => setForm({ ...form, typeExamen: e.target.value })} className={cls} required>
