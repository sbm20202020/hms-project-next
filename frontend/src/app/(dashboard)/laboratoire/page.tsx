...
import { useExamensLabo, useUpdateExamenLabo } from '@/hooks/use-labo'
import { Modal } from '@/components/ui/modal'
import { ExamenLaboForm } from '@/components/forms/examen-labo-form'
import { Plus, Loader2, Search, FlaskConical, Clock, CheckCircle } from 'lucide-react'
import { toast } from 'sonner'

export default function LaboratoirePage() {
  const { data: examens = [], isLoading } = useExamensLabo()
  const updateExamen = useUpdateExamenLabo()
  ...
  const filtered = list.filter((e: any) => {
    ...
    const matchSearch = patient.toLowerCase().includes(search.toLowerCase()) || (e.type_examen || '').toLowerCase().includes(search.toLowerCase())
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
          <div className="flex items-center gap-2"><FlaskConical className="h-4 w-4 text-blue-500" /><p className="text-sm text-muted-foreground">En cours</p></div>
                ...
                {filtered.length ? filtered.map((e: any) => {
                  ...
                  return (
                    <tr key={e.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-muted-foreground">{e.type_examen || '-'}</td>
      ...
      <Modal open={showCreate} onClose={() => setShowCreate(false)} title="Nouvel examen de laboratoire">
        <ExamenLaboForm onSuccess={() => setShowCreate(false)} />
      </Modal>
    </div>
  )
}
