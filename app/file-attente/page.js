          ...
          <Card className="lg:col-span-1">
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                Numéro Actuel
                <Badge variant={isActive ? "default" : "secondary"}>{isActive ? "Actif" : "Pause"}</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-center">
                <div className="text-6xl font-bold text-cyan-600 mb-2">{currentNumber}</div>
                <p className="text-gray-600 mb-4">Consultation Générale</p>
                <div className="text-sm text-gray-500">Appelé à 10:28</div>
              </div>
            </CardContent>
          </Card>
                ...
                {queueData.map((item, index) => (
                  <div key={item.number} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <Badge variant={item.status === "priority" ? "destructive" : "secondary"}>
                        {item.status === "priority" ? "Priorité" : "En attente"}
                      </Badge>
                    </div>
                  </div>
                ))}
